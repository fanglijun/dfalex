...
class RegexParser {
    ...
    private char _parseCharEscape()
    {
        ...
        try
        {
            ...
            switch(c)
            {
                ...
                case 'x':
                case 'u':
                return(char)Integer.parseUnsignedInt(m_src.substring(spos+1, m_readPos), 16);
    ...
    private static DfaState<Action> _buildParserDfa() //produces A
    {
        ...
        //A: literal | .
        bld.addPattern(Apos.then(":").then(CharRange.builder().addChars(".()[]+*?|\\").invert().build()), x-> {
            CharRange range;
            char c = x._lastChar();
            if (!x.m_caseI)
            {
        ...
        bld.addPattern(Apos.then(charEscape), x -> x._push("A",CharRange.single(x._parseCharEscape())));
        ...
        //class chars
        bld.addPattern(cpos.then(":").then(CharRange.builder().addChars("-[]\\").invert().build()), x->{
            x.m_cprev = x.m_clast;
